...
const linkPageSlice = createSlice({
  ...
  reducers: {
    ...
    reorderLinkPages(state, action: PayloadAction<{ fromIndex: number; toIndex: number }>) {
      const { fromIndex, toIndex } = action.payload
      const [movedItem] = state.splice(fromIndex, 1)
      state.splice(toIndex, 0, movedItem)
    },
    addLinkGroup(state, action: PayloadAction<{ pageIndex: number; group: LinkGroup }>) {
    ...
    deleteLinkGroup(state, action: PayloadAction<{ pageIndex: number; groupIndex: number }>) {
    ...
    reorderLinkData(
      ...
      const [movedItem] = state[pageIndex].groupList[groupIndex].linkList.splice(fromIndex, 1)
      state[pageIndex].groupList[groupIndex].linkList.splice(toIndex, 0, movedItem)
    },
  },
})
