...
const LinkDialog = ({
  ...
  const existingNames = linkPages[pageId].groupList[panelId].linkList.map((lnk) =>
    lnk.text.trim().toLowerCase(),
  )

  const urlValid = validUrl(url)
  const isDuplicate = link.text != name && existingNames.includes(name?.trim().toLowerCase())
  ...
  const handleClose = (confirm: boolean) => {
    if (!confirm) {
      ...
      onClose({ text: name, url: url, color: color, outline: outlined, icon: icon }, false)
    }
    resetDialog()
  }
  ...
  return (
    <Dialog
      ...
      onClose={handleClose}
      disableConfirm={
        !name || !url || urlValid !== true || isDuplicate || (editMode && !hasChanged)
      }
      actionButton={
        editMode
          ? {
